...
const CELL_SUB: React.CSSProperties = {
  ...FONT_BASE,
  fontSize: 'var(--text-xs)',
  fontWeight: 400,
  ...
  return (
    <button
      ...
      onMouseEnter={e => {
        ...
        e.currentTarget.style.background = 'color-mix(in srgb, var(--primary) 5%, var(--card))';
      }}
      onMouseLeave={e => {
...
function ReceiptRow({
  ...
  onChangeQty: (itemId: string, newQty: number) => void;
}) {
  const fmt = (n: number) =>
    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0, padding: '5px 10px', display: 'flex', flexDirection: 'column', justifyContent: 'center', overflow: 'hidden' }}>
          ...
          {receipt.projectName && (
            <span style={CELL_SUB}>Prosjekt: {receipt.projectName}</span>
          )}
        </div>

        {/* Total */}
        <div style={{ width: 100, flexShrink: 0, padding: '5px 10px', textAlign: 'right', ...CELL_PRIMARY }}>
          {fmt(receipt.totalAmount)}
        </div>

        {/* Chevron */}
        <div style={{ width: 36, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <ChevronSvg open={receipt.expanded} />
        </div>
      </button>
        ...
        <div style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', height: 30, borderBottom: '1px solid var(--border)', paddingBottom: 5 }}>
            ...
            <div style={{ flex: 1, minWidth: 0, padding: '0 10px' }}>
              <span style={LABEL_COL}>Varenavn</span>
            </div>
            <div style={{ width: 195, flexShrink: 0, padding: '0 10px 0 61px' }}>
              <span style={LABEL_COL}>Antall</span>
            </div>
            <div style={{ width: 90, flexShrink: 0, padding: '0 10px', display: 'flex', justifyContent: 'flex-end' }}>
              <span style={LABEL_COL}>Per enhet</span>
            </div>
            <div style={{ width: 68, flexShrink: 0, padding: '0 10px' }}>
              <span style={LABEL_COL}>Rabatt</span>
            </div>
            <div style={{ width: 93, flexShrink: 0, padding: '0 32px 0 10px', display: 'flex', justifyContent: 'flex-end' }}>
              <span style={LABEL_COL}>Total</span>
            </div>
          </div>
          ...
          {receipt.items.map((item, idx) => (
            <div
              ...
              style={{
                ...
                minHeight: 67,
                background: 'var(--card)',
                borderBottom: idx < receipt.items.length - 1 ? '1px solid var(--border)' : 'none',
                position: 'relative',
              }}
            >
              {/* Checkbox */}
              ...
              <div style={{ width: 56, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '5px 10px' }}>
                <img
                  src={item.image}
                  alt=""
                  style={{ width: 36, height: 36, borderRadius: 'var(--radius)', objectFit: 'cover', display: 'block' }}
                />
              </div>

              {/* Name */}
              <div style={{ flex: 1, minWidth: 0, padding: '5px 10px' }}>
                <span style={{ ...FONT_BASE, fontSize: 'var(--text-base)', fontWeight: 500, color: 'var(--foreground)', lineHeight: 1.4 }}>
                  {item.name}
                </span>
              </div>
              ...
              <div style={{ width: 195, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10, padding: '5px 10px' }}>
                ...
                <span style={{ ...FONT_BASE, fontSize: 'var(--text-base)', fontWeight: 400, color: 'var(--foreground)' }}>
                  {item.unit}
                </span>
              </div>
              ...
              <div style={{ width: 93, flexShrink: 0, padding: '5px 32px 5px 10px', textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'center' }}>
                <span style={{ ...FONT_BASE, fontSize: 'var(--text-base)', fontWeight: 500, color: 'var(--foreground)' }}>
                  {fmt(item.total)}
                </span>
                {item.discountPct > 0 && (
                  <span style={{ ...FONT_BASE, fontSize: 'var(--text-base)', fontWeight: 500, color: 'var(--secondary-foreground)' }}>
                    {fmt(item.originalTotal)}
                  </span>
                )}
              </div>
            </div>
...
interface PreviousPurchasesModalProps {
  onClose: () => void;
  customerName?: string;
  onAddAsNormalSale?: (items: Array<{ name: string; quantity: number; unit: string; price: number; discount: number; total: number }>) => void;
  onAddAsReturn?: (items: Array<{ name: string; quantity: number; unit: string; price: number; discount: number; total: number }>) => void;
}

export function PreviousPurchasesModal({ onClose, customerName, onAddAsNormalSale, onAddAsReturn }: PreviousPurchasesModalProps) {
  ...
  // Close on Escape; auto-focus search input; flag body so TopMenu hides
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose(); };
    document.addEventListener('keydown', onKey);
    document.body.setAttribute('data-previous-purchases-open', 'true');
    setTimeout(() => inputRef.current?.focus(), 60);
    return () => {
      document.removeEventListener('keydown', onKey);
      document.body.removeAttribute('data-previous-purchases-open');
    };
  }, [onClose]);
  ...
  const handleSearch = () => {
    ...
    // Any bong number 110-3 through 110-7 randomly triggers one of the two receipts
    ...
    // Fallback: exact match on bongNumber or receiptNumber
    const matched = MOCK_RECEIPTS.filter(r =>
      r.bongNumber.toLowerCase() === term || r.receiptNumber.toLowerCase() === term
    );
    setReceipts(matched.map(r => ({
      ...r,
      expanded: true,
      items: r.items.map(i => ({ ...i, checked: false })),
    })));
  };

  const toggleExpand = (id: string) =>
    setReceipts(prev => prev.map(r => r.id === id ? { ...r, expanded: !r.expanded } : r));

  const toggleCheck = (receiptId: string, itemId: string) =>
    setReceipts(prev => prev.map(r => {
      if (r.id !== receiptId) return r;
      return { ...r, items: r.items.map(i => i.id === itemId ? { ...i, checked: !i.checked } : i) };
    }));

  const changeQty = (receiptId: string, itemId: string, newQty: number) =>
    setReceipts(prev => prev.map(r => {
      if (r.id !== receiptId) return r;
      return { ...r, items: r.items.map(i => i.id === itemId ? { ...i, quantity: newQty } : i) };
    }));
    ...
    >
      ...
      <div style={{ background: 'var(--card)', borderBottom: '1.5px solid var(--border)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '18px 20px' }}>
          <HistoryIconSvg />
          <span style={{ ...FONT_BASE, fontSize: 'var(--text-lg)', fontWeight: 700, color: 'var(--foreground)', lineHeight: 1.3 }}>
            Finn tidligere salg
          </span>
        </div>
      </div>
      ...
      <div
        style={{
          ...
          height: 45,
          flexShrink: 0,
          boxShadow: '0 3px 3px rgba(107,107,114,0.06)',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', height: '100%', padding: '0 20px', gap: 30 }}>
          <button
            style={{
              ...
              padding: '0 0 5px 0',
              cursor: 'pointer', whiteSpace: 'nowrap', lineHeight: 1.75,
            }}
          >
            Alle salg
          </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 15, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          {/* Bong number label + input */}
          <div style={{ marginBottom: 0 }}>
            <label style={{ ...FONT_BASE, display: 'block', fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--foreground)', lineHeight: 1.75, marginBottom: 4 }}>
              Bongnummer
            </label>
            <div style={{ position: 'relative', width: 320 }}>
              <input
                ...
                style={{
                  ...
                  border: bongNumber ? '2px solid var(--primary)' : '1px solid var(--border)',
                  borderRadius: 'var(--radius)',
                  ...
                  boxShadow: 'inset 2px 2px 3px rgba(0,0,0,0.06)',
              ...
              <button
                ...
                style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  padding: 4, color: 'var(--foreground)', opacity: 0.5,
                }}
              >
                <SearchIconSvg />
              </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 48 }}>
            ...
            <span style={{ ...FONT_BASE, fontSize: 'var(--text-sm)', fontWeight: 400, color: 'var(--foreground)', lineHeight: 1.4 }}>
              Kunde uten konto
            </span>
          </div>
          ...
          {hasResults && (
            <div style={{ display: 'flex', alignItems: 'flex-end', height: 28, marginBottom: 12 }}>
              ...
              <div style={{ width: 160, flexShrink: 0, padding: '0 10px' }}>
                <span style={LABEL_COL}>Kvitteringsdetaljer</span>
              </div>
              <div style={{ flex: 1, minWidth: 0, padding: '0 10px' }}>
                <span style={LABEL_COL}>Kunde, prosjekt</span>
              </div>
              <div style={{ width: 100, flexShrink: 0, padding: '0 10px', display: 'flex', justifyContent: 'flex-end' }}>
                <span style={LABEL_COL}>Totalbeløp</span>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
              {receipts.map(receipt => (
                <ReceiptRow
                  ...
                  onToggleCheck={itemId => toggleCheck(receipt.id, itemId)}
                  onChangeQty={(itemId, qty) => changeQty(receipt.id, itemId, qty)}
                />
              ))}
            </div>
          )}
      ...
      >
        <FooterBtn
          variant={anyChecked ? 'primary' : 'muted'}
          onClick={() => {
            if (!anyChecked) return;
            const checkedItems = receipts.flatMap(r =>
              r.items.filter(i => i.checked).map(i => ({
                name: i.name,
                quantity: i.quantity,
                unit: i.unit,
                price: i.perUnit,
        ...
        <FooterBtn
          variant={anyChecked ? 'outlined' : 'muted'}
          onClick={() => {
            if (!anyChecked) return;
            const checkedItems = receipts.flatMap(r =>
              r.items.filter(i => i.checked).map(i => ({
            ...
            onAddAsNormalSale?.(checkedItems);
            onClose();
          }}
        >
          Legg til som normalt salg
        </FooterBtn>
        <FooterBtn variant={anyChecked ? 'outlined' : 'muted'}>
          Skriv ut byttelapp
        </FooterBtn>
        <FooterBtn variant="outlined" onClick={onClose}>
          Avbryt
        </FooterBtn>
      </div>
    </div>
